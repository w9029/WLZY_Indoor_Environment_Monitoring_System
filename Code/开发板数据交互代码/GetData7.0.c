...
#define DESIP "47.99.195.202"
//#define DESIP "192.168.100.61"
#define DESPORT 8880
...
void *VideoSwitch()
{
	...
	if(videofd < 0)
	{
		perror("VideoSwitch:socket error\n");
		return NULL;
	}

	seraddr.sin_family = AF_INET;
	seraddr.sin_port = htons(VIDEOPORT);
	seraddr.sin_addr.s_addr = inet_addr(VIDEOIP);
	
	ret = -1;
	while(ret < 0)
	{
		ret = bind(videofd, (struct sockaddr*)&seraddr, sizeof(struct sockaddr));
		if(ret < 0)
		{
			perror("VideoSwitch:bind error\n");
			close(videofd);
			return NULL;
		}
		sleep(5);	
	}
	printf("VideoSwitch:bind ok\n");
	...
	while(1)
	{
		cfd = accept(videofd, (struct sockaddr*)&seraddr, &len);
		if(cfd < 0)
		{
			perror("VideoSwitch:accept error\n");
			continue;
		}
		printf("VideoSwitch:link ok\n");
		...
		printf("VideoSwitch: said:%s\n",buff);
		if(strcmp(buff, "on") == 0)
		{
			printlog(2, time(NULL), "Camera", "got camera open request");
			pid = fork();
			if(pid == 0)
			{
				system("bash /workspace/VideoSwitch/open.sh");
				printf("VideoSwitch's child die with 0\n");
				exit(0);
			}
		}
		else if(strcmp(buff, "off") == 0)
		{
			printlog(2, time(NULL), "Camera", "got camera close request");
			system(". /workspace/VideoSwitch/close.sh&");
		}
...
void *connecttest()
{
	...
	while(1)
	{
		...
		else
		{
			...
			if(pid == 0)
			{
				system("bash /workspace/SetTime/settime.sh >/dev/null 2>&1 &");
				exit(0);
			}
		}
		sleep(5);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	pthread_create(&tid3, NULL, VideoSwitch, NULL);
